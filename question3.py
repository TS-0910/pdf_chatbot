...
def main(url, first, finish):    
    ...
    #空白を詰める。    
    if first:
        print(''.join(f"SYSTEM : こんにちは！{custom_summary}何かお手伝い出来ることはありますか？".split()))
        first = False

    else:
        print(''.join(f"SYSTEM : {custom_summary}こちらについては何かお手伝い出来ることはありますか？".split()))        
        
    #Q&A
    qanda(knowledge_base, llm, first, finish)
